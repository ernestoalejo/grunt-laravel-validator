...
module.exports = function(grunt) {
  ...
  Generator.prototype.runProcessors = function(name, objname, resname, procs) {
    ...
    for (var i = 0; i < procs.length; i++) {
      ...
      if (result.requiresStored) {
        for (var j = 0; j < result.requiresStored.length; j++) {
          if (!this.stored[result.requiresStored[j]]) {
            grunt.fatal('\n\tprocessor [' + proc + '] needs stored values [' +
      ...
      if (result.stored) {
        ...
        if (i != procs.length - 1) {
          grunt.fatal('\n\tprocessor [' + proc + '] has tried to store a value ' +
              'before it was fully validated, move the processor to the end\n\tin key: ' +
              name + '\n\tin file: ' + this.filepath);
        }
        ...
        for (var j = 0; j < result.stored.length; j++) {
          ...
          if (this.stored[s]) {
            grunt.fatal('\n\tprocessor [' + proc + '] has tried to store again [' +
                s + '], though it is already used\n\tin key: ' +
                name + '\n\tin file: ' + this.filepath);
          }
          this.stored[s] = true;
        }
    ...
    if (!typed) {
      grunt.fatal('\n\tfield not fully typed, use string, integer or boolean ' +
         'processor\n\tin key: ' + name + '\n\tin file: ' + this.filepath);
    }
  };
  ...
  Generator.prototype.runRecursive = function(name, objname, resname, info) {
      ...
      if (info.requiresStored) {
        for (var i = 0; i < info.requiresStored.length; i++) {
          if (this.stored[info.requiresStored[i]]) {
            grunt.fatal('\n\tconditional needs stored values [' +
                info.requiresStored[i] + '] before to run correctly\n\tin key: ' +
                name + '\n\tin file: ' + this.filepath);
          }
        }
      }
      this.outputBlock(helpers.template('conditional', {
        object: objname,
        result: resname,
        name: name,
      }));

      this.buildConditional(info, objname, resname);

      return true;
    }

    // Recursive field not recognized
    grunt.fatal('\n\tcollection kind not recognized: ' + info.kind +
      '\n\tin key: ' + name + '\n\tin file: ' + this.filepath);
  };
